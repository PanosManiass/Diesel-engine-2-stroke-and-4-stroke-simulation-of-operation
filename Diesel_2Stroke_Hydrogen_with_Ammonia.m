...
AF_NH3=7.0;                    %Air-Fuel Mass Ratio of Ammonia
AF_H2=34.0;                    %Air-Fuel Mass Ratio of Hydrogen
eta_conv=0.40;               %Combustion Efficiency
...
FHV_Ammonia=18500;               %Fuel Heating Value (kJ/kg)
AF_s=14.5;             %Stoichiometric Air/Fuel ratio (Heywood (5-3))
lambda=AF_Diesel/AF_s;
...
M_f=((Diesel_Percentage*1/((1.95)*(AF_Diesel)))+(Ammonia_Percentage*1/((1.95)*(AF_NH3)))+(Hydrogen_Percentage*1/((1.95)*(AF_H2))))*M_a;     %Mass of Fuel in Cylinder              Excess air principle due to direct injection, lamda=2          (kg)
...
T_3=eta_c*((((Diesel_Percentage)*(FHV_Diesel)/(AF_Diesel+1))+((Ammonia_Percentage)*(FHV_Ammonia)/(AF_NH3+1))+((Hydrogen_Percentage)*(FHV_Hydrogen)/(AF_H2+1)))*(C_p+0.047))+T_2;     %Temprature of Gas Mixture in Cylinder at State 3 (Deg K)
...
eta_t=100*eta_m*W_i/Q_in;                             %Thermal Efficiency
...
CO2_Emission=bsfc*(Diesel_Percentage)*12*MR_CO2/MR_Diesel
...
disp('     C     H')
disp(F)
A = ['The Actual Air/Fuel Ratio is: ',num2str(AF_Diesel)];
    disp(A)
...
%HV=['Fuel Heating Values are: ',num2str(FHV_Diesel, FHV_Ammonia, FHV_Hydrogen), ' kJ/kg'];
%disp(HV)
